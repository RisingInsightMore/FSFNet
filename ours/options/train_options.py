# --*--coding   : uft-8  --*--
# @Time         : 2026/2/15 10:42
# @Author       : RisingInsight
# @File         : train_options2.py
# @Software     : PyCharm
# @Project      : FSFNet-main
# @Function     :
...
class TrainOptions(BaseOptions):
    def initialize(self):
        ...
        self.parser.add_argument('--gradient_penalty', action='store_true', help='use gradient_penality')       # 是否使用梯度惩罚（WGAN-GP等对抗训练技巧）
        ...
        self.parser.add_argument('--pretrain_dir', type=str, default='',
                                 help='path to pretrained model directory (e.g., ./checkpoints/FSFNet_Pretrain/FSFNet_Pretrain0)')
        ...
        self.parser.add_argument('--use_wavelet_loss', action='store_true', help='Enable wavelet style loss in training')
        self.parser.add_argument('--lambda_aux', type=float, default=0.2, help='weight for auxiliary frequency reconstruction loss')
        self.parser.add_argument('--use_granular', action='store_true', help='enable granular ball module')
        self.isTrain = True
