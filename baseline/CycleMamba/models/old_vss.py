...
if WITH_TRITON:
    try:
        ...
        warnings.warn("if you are using py37, add this line to functools.py: ""cached_property = lambda func: property(lru_cache()(func))")
...
class Linear2d(nn.Linear):
    ...
    def remove_prefix(self, state_dict, prefix='module.'):
        """
        Recursively removes a prefix from all keys in a state dictionary.
        """
        new_state_dict = {}
    ...
    def remove_module_prefix(self, key, prefix='module'):
        """
        Removes the 'module' prefix from a string if it exists.
    
    Parameters:
    - key (str): The string from which to remove the prefix.
    - prefix (str): The prefix to remove. Defaults to 'module'.
    
    Returns:
    - str: The string with the 'module' prefix removed, if it existed.
    """
        if key.startswith(prefix + '.'):
            return key[len(prefix) + 1:]  # +1 to remove the dot after 'module'
        elif key.startswith(prefix):
            return key[len(prefix):]  # Remove only the prefix if no dot follows
        else:
            return key 
    
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict, missing_keys, unexpected_keys, error_msgs):
        # print(prefix)  
        state_dict[prefix + "weight"] = state_dict[prefix + "weight"].view(self.weight.shape)       # state_dict[prefix + "weight"]取出前缀prefix+weight的具体值
...
class VSSBlock(nn.Module):
    def __init__(
            ...
            ssm_init="v0",              # 用的是v0-v2  然后交换两次维度
            forward_type="v02",            # 改过       v2 和 m0_noz        v0和v05_noz
            # =============================
            mlp_ratio=4.0,
    ...
    def _forward(self, input: torch.Tensor):
        ...
        if self.ssm_branch:
            if self.post_norm:
                ...
                # print(x.shape)      # torch.Size([1, 80, 80, 128])
...
class _VSSBlock(nn.Module):
    def __init__(
        ...
        channel_first=False,
        # =============================
        ssm_d_state: int = 16,              # 1111
        ssm_ratio=2.0,      
        ssm_dt_rank: Any = "auto",
        ssm_act_layer=nn.SiLU,          ###  nn.GELU,
        ssm_conv: int = 3,
        ssm_conv_bias=True,
        ssm_drop_rate: float = 0,               #######
        ssm_init="v0",
        forward_type="v02",             # v05_noz
        # =============================
        mlp_ratio=4.0,
        mlp_act_layer=nn.GELU,
        mlp_drop_rate: float = 0.0,             #####  nn.GELU,
        gmlp=False, 
        # =============================
        use_checkpoint: bool = False,
        post_norm: bool = False,
