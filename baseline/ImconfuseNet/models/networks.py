...
class Encoder(nn.Module):
    def __init__(
        self, in_channels, base_ch, num_res_blocks, latent_channels, ch_mult=(1,1)
    ):
        super().__init__()
        layers = []
        layers.append(nn.Conv2d(
            in_channels, base_ch, kernel_size=3, stride=1, padding=1,
        ))
        in_channels = base_ch
        for mul in ch_mult:
            out_channels = base_ch * mul
            for _ in range(num_res_blocks):
                layers.append(ResnetBlock1(in_channels, out_channels=out_channels))
                in_channels = out_channels
            #layers.append(Downsample(in_channels))
        layers.append(ResnetBlock1(in_channels))
...
class Decoder(nn.Module):
    def __init__(
        ...
        layers.append(nn.Conv2d(
            latent_channels, in_channels, kernel_size=3, stride=1, padding=1,
        ))
        layers.append(ResnetBlock1(in_channels))
        layers.append(AttnBlock(in_channels))
        layers.append(ResnetBlock1(in_channels))
        ...
        layers.append(Normalize(in_channels))
        layers.append(nn.Conv2d(
            in_channels, out_channels_, kernel_size=3, stride=1, padding=1,
        ))
        self.layers = nn.Sequential(*layers)
...
class Network(nn.Module):
    def __init__(
        self, latent_channels=10, in_channels=3, out_channels_rgb=3, out_channels_hsi=20,
        base_ch=32, num_res_blocks=2, 
    ):
        super().__init__()
        self.encoder = Encoder(in_channels, base_ch, num_res_blocks, latent_channels)
        self.decoder_hsi = nn.Conv2d(latent_channels, out_channels_hsi, 11, padding=5)
        self.softmax=nn.Softmax(dim=1)

    def forward(self, input):
        aunbance = self.encoder(input)
        aunbance = aunbance-torch.max(aunbance)
        aunbance = torch.clamp(aunbance, -20, -0.001)
        aunbance=self.softmax(aunbance)
        dec_hsi = self.decoder_hsi(aunbance)
        return aunbance, dec_hsi