...
def define_LIIF(n_colors, encoder_spec = 'rdn', imnet_spec = 'mlp', inp_size = 48, norm='batch', init_type='normal', init_gain=0.02, gpu_ids=[]):
    """Create a generator
    ...
    # net = liif_2d.LIIF(n_colors, encoder_spec, imnet_spec, inp_size)
    net = liif.LIIF(n_colors, encoder_spec, imnet_spec, inp_size)
...
class ResnetGenerator(nn.Module):
    ...
    def forward(self, input):
        """Standard forward"""
        down_res_input = self.model(input)
        #ca_input = self.ca(down_res_input) * down_res_input
        #sa_input = self.sa(ca_input) * ca_input
        model2_input = self.model2(down_res_input)
        #ca_input2 = self.ca1(model2_input) * model2_input
        #sa_input2 = self.sa1(ca_input2) * ca_input2
        output = self.model3(model2_input)
...
class ResnetExtractor(nn.Module):
    ...
    def forward(self, input):
        ...
        # Spatial attention
        avg_out = torch.mean(h_featuremap, dim=1, keepdim=True)
        max_out, _ = torch.max(h_featuremap, dim=1, keepdim=True)
        x = torch.cat([avg_out, max_out], dim=1)
        x = self.conv1(x)
        mask = self.sigmoid(x)
...
def eqHist(input_nc):
    input_nc = input_nc*255
    input_nc = input_nc.type(torch.uint8)
    input_nc = input_nc.cpu().numpy()
    clahe = cv2.createCLAHE(clipLimit=5.0, tileGridSize=(8,8))
    (b,g,r) = cv2.split(input_nc) #通道分解
    bH = clahe.apply(b)
    gH = clahe.apply(g)
    rH = clahe.apply(r)
    result = cv2.merge((bH,gH,rH),)#通道合成
    img_dst_clahe = transforms.ToTensor()(result)
    img_dst_clahe = transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))(img_dst_clahe)
