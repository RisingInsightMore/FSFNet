...
class AECMModel(BaseModel):
    ...
    def calculate_NCE_loss(self, src, tgt):
        ...
        # DataParallel may wrap list output in an extra list
        if isinstance(feat_q, list) and len(feat_q) > 0 and isinstance(feat_q[0], list):
            feat_q = feat_q[0]
        if isinstance(feat_k, list) and len(feat_k) > 0 and isinstance(feat_k[0], list):
            feat_k = feat_k[0]
        # 加个权重
        feat_q_w = []
        feat_k_w = []
    ...
    def calculate_temporal_loss(self, real, fake):
        ...
        if isinstance(feats_q, list) and len(feats_q) > 0 and isinstance(feats_q[0], list):
            feats_q = feats_q[0]
        if isinstance(feats_k, list) and len(feats_k) > 0 and isinstance(feats_k[0], list):
            feats_k = feats_k[0]
