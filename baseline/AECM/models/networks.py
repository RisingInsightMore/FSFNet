    ...
    def create_mlp(self, feats):
        for mlp_id, feat in enumerate(feats):
            if isinstance(feat, list):
                feat = feat[0]  # Or aggregate as needed
            input_nc = feat.shape[1]
...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect', no_antialias=False, no_antialias_up=False, opt=None):
        ...
        # self.m1 = nn.Conv2d(64, 128, kernel_size=3, stride=1, padding=1, bias=use_bias)
        # self.m2 = norm_layer(128)
        # self.m3 = nn.ReLU(True)
        # self.m4 = Downsample(128)
        # self.m5 = nn.Conv2d(128, 256, kernel_size=3, stride=1, padding=1, bias=use_bias)
        # self.m6 = norm_layer(256)
        # self.m7 = nn.ReLU(True)
        # self.m8 = Downsample(256)
    ...
    def forward(self, input, layers=[], encode_only=False):
        ...
        if len(layers) > 0:
            ...
            # m = self.m7(m)
            # m = self.m8(m)
            fake = self.model2(fake)
            fake = self.model3(fake)
            return fake
...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(32 * 8 * 8, 128)  # 计算特征图大小为8x8
        self.fc2 = nn.Linear(128, 10)

class MDiscriminator(nn.Module):
    def __init__(self, in_feat=576, hid_feat = 768, out_feat = 768, dropout = 0.):
        super().__init__()
        if not hid_feat:
            hid_feat = in_feat
        if not out_feat:
            out_feat = in_feat
        self.conv1 = nn.Conv2d(in_channels=3, out_channels=16, kernel_size=3, stride=3, padding=0)
        self.conv2 = nn.Conv2d(in_channels=3, out_channels=16, kernel_size=3, stride=3, padding=0)
...
class D_attn(nn.Module):
    """Attention-based discriminator for ViT tokens.
    Input: tokens [B, N, 768]
    Output: (logit [B, 1], cam_logit [B, N, 1])
    """
    def __init__(self, in_dim=768, hid_dim=256):
        super().__init__()
    ...
    def forward(self, x):
        # x: [B, N, C]
        logit = self.mlp(x).mean(dim=1)  # [B, 1]
        cam_logit = self.cam(x)  # [B, N, 1]
        return logit, cam_logit


class NCE_attn(nn.Module):
    """Channel attention for NCE feature weighting.
    Creates separate attention modules for each channel dimension.
    Input: q [B, C, H, W], k [B, C, H, W]
    Output: (q_w [B, C, H, W], k_w [B, C, H, W])
    """
    def __init__(self, channel_dims=None):
        super().__init__()
        self.channel_dims = channel_dims or [64, 128, 256]
        self.gap = nn.AdaptiveAvgPool2d(1)
        self.fc_q = nn.ModuleDict()
        self.fc_k = nn.ModuleDict()
        for c in self.channel_dims:
            c_str = str(c)
            self.fc_q[c_str] = nn.Sequential(
                nn.Linear(c, max(c // 4, 1)),
                nn.ReLU(inplace=True),
                nn.Linear(max(c // 4, 1), c),
                nn.Sigmoid()
            )
            self.fc_k[c_str] = nn.Sequential(
                nn.Linear(c, max(c // 4, 1)),
                nn.ReLU(inplace=True),
                nn.Linear(max(c // 4, 1), c),
                nn.Sigmoid()
    ...
    def forward(self, q, k):
        ...
        c = q.shape[1]
        c_str = str(c)
        if c_str not in self.fc_q:
            # Fallback: create on-the-fly (won't be optimized, but won't crash)
            self.fc_q[c_str] = nn.Sequential(
                nn.Linear(c, max(c // 4, 1)), nn.ReLU(inplace=True),
                nn.Linear(max(c // 4, 1), c), nn.Sigmoid()
            ).to(q.device)
            self.fc_k[c_str] = nn.Sequential(
                nn.Linear(c, max(c // 4, 1)), nn.ReLU(inplace=True),
                nn.Linear(max(c // 4, 1), c), nn.Sigmoid()
            ).to(q.device)
        q_w = self.gap(q).flatten(1)  # [B, C]
        q_w = self.fc_q[c_str](q_w).unsqueeze(-1).unsqueeze(-1)  # [B, C, 1, 1]
        k_w = self.gap(k).flatten(1)
        k_w = self.fc_k[c_str](k_w).unsqueeze(-1).unsqueeze(-1)
        return q * q_w, k * k_w
