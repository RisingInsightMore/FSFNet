...
class dataset_single(data.Dataset):
    ...
    def __getitem__(self, index):
        data, file_name = self.load_img(self.images[index], self.input_dim)
        return data, file_name

    def load_img(self, img_name, input_dim):
        file_name = img_name
        img_path = os.path.join(self.dataroot, self.phase + self.setname, img_name)
        img = Image.open(img_path).convert('RGB')
        img = self.transforms(img)
        if input_dim == 1:
            img = img[0, ...] * 0.299 + img[1, ...] * 0.587 + img[2, ...] * 0.114
            img = img.unsqueeze(0)
        # sanitize: a corrupt image can decode to NaN/Inf and poison the whole
        # model. Zero it (and warn) instead of feeding garbage into the network.
        if torch.isnan(img).any() or torch.isinf(img).any():
            print('[WARN] NaN/Inf in image -> zeroed: %s' % img_path)
...
class dataset_unpair(data.Dataset):
    ...
    def load_img(self, img_name, input_dim):
        ...
        # model. Zero it (and warn) instead of feeding garbage into the network.
        if torch.isnan(img).any() or torch.isinf(img).any():
            print('[WARN] NaN/Inf in image -> zeroed: %s' % img_name)
            img = torch.zeros_like(img)
        return img
